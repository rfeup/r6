...
begin
    ...
    count:=NetGetWord(NetInBufC);
    for  i:=0 to (count - 1) do begin
       ImageCol[i]:= NetGetWord(NetInBufC);
       ImageR[i]:= NetGetWord(NetInBufC);
       ImageG[i]:= NetGetWord(NetInBufC);
       ImageB[i]:= NetGetWord(NetInBufC);


//       FHal.EMission.Text:=IntToSr();
    end;
    //writeln(ImageCol[4]);
...
begin
    ...
    StartButton := (NetGetWord(NetInBufOdo) = 1);
end;
...
begin
  ...
  if StartButton then begin
    ShapeStartButton.Brush.Color := clGreen;
  end else begin
    ShapeStartButton.Brush.Color := clRed;
  end;
  ...
  if StartButton then begin
    ...
    end else if FHal.RGMission.ItemIndex>0 then begin
         writeln('merda3');
        FHal.TWaitAtStart.Enabled:=true;
  ...
  //Ao carregar no Start Button, lança um timer de 3 segundos e faz Build Path (atWait);
  ...
  //EditRobotX.Text := format('%.3g', [RobotState.x]);
  //EditRobotY.Text := format('%.3g', [RobotState.y]);
  //EditRobotTheta.Text := format('%.5g', [radtodeg(RobotState.teta)]);
