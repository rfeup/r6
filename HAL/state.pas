...
*)
  ...
  sourceX = -1.12;//-1.12 //-1.13;
  targetX = 1.11;//1.11 //1.13

  clawCenterDistance = 0.31;//0.3

  warehouseHallwayY = 0.53;

  warehouseLeaveSpeed = 0.2;//0.25
  defaultFinalSpeed = 0;
...
begin
  ...
  vy := -1 * rpy;
  w := -(StrToFloat(FHal.EWGoToXY.Text)) * error_teta;
...
begin
 ...
 (* ang := abs(DiffAngle(action.path_teta, action.teta));
  ...
  vx := speed;
  vy := -1 * rpy;    //2
  w := -(StrToFloat(FHal.EWRotateAndGo.Text)) * error_teta;     //1.5
...
begin
  ...
  vx := speed;
  vy := -1.5 * rpy;        //2.5
  w := -(StrToFloat(FHal.EWGoToWarehouse.Text)) * error_teta;       //2.5
...
begin
 ...
 // writeln(CurStep);
  case ActionScript[CurStep].atype of
      ...
      atStabilize: begin
      FHal.EMission1.Text:='Seu Bandalho';
      count := count + 1;
      v := 0;
      vn := 0;
      w := 0;
...
begin
 ...
 { asAddGotoXY(-1.11, 0.6, pi/2, pi/2, 0.15);
  ...
  //Inc(ActionScriptCount);
     asAddGotoXY(-1.10, 0.45, rad(90), ATan2(0.45-(-0.52),-1.10-(-1.10)), 0.3);
     asAddGotoXY(-0.75, 0.45, rad(90), ATan2(0.45-(0.45),-0.75-(-1.10)), 0.3);
     with ActionScript[ActionScriptCount] do begin
         atype := atStabilize;
     end;
     Inc(ActionScriptCount);
     FHal.TimerToCheckColor.Enabled:=True;
  end else begin
     with ActionScript[ActionScriptCount] do begin
         atype := atWait;
     end;
